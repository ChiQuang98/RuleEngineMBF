...
<script type="text/html" data-template-name="mqtt in">
    ...
    <div class="form-row">
        ...
        <select name="cars" id="node-input-topic">
            <!--            <option value="h2/s/#"> h2/s/#</option>-->
        </select>
    </div>
    <div class="form-row">
...
<!--    <script src="https://cdnjs.cloudflare.com/ajax/libs/crypto-js/3.1.2/components/aes-min.js"/>-->
    <script type="text/javascript">
        ...
        var token = document.cookie
            ...
            .find(row => row.startsWith('TokenUser'))
            .split('=')[1];
        // console.log("TOKENCLIENT: "+document.cookie
        //     .split('; ')
        //     .find(row => row.startsWith('TokenUser'))
        //     .split('=')[1])
        var loadData = function () {
            $.ajax({
                type: 'GET',
                url: 'http://10.16.150.139:8000/getChannelsByUser/'+'Bearer '+token,
                success: function(response) {
                    console.log("QUANGLISTCHANNELS:"+response);
                    var listChannel = response.data
                    for (var i = 0; i < listChannel.length; i++) {
                        var select = document.getElementById("node-input-topic");
                        for (var i = 0; i < listChannel.length; i++) {
                            var el = document.createElement("option");
                            el.textContent = listChannel[i].channel_name;
                            el.value = "channels/" + listChannel[i].channel_id + "/messages";
                            select.appendChild(el);
                        }
                        document.getElementById("node-input-name").value = "Mobifone Channel In";
                        var a = document.getElementById('node-input-topic');
                        $('#node-input-topic').change(function () {
                            // alert(this.value);
                            // console.log(a.selectedIndex);
                            var username = listChannel[a.selectedIndex].thing_id;
                            var password = listChannel[a.selectedIndex].thing_key;
                            $.ajax({
                                type: 'GET',
                                url: 'http://10.16.150.139:8000/encrypt/'+password,
                                success: function(response) {
                                    // console.log(response);
                                    setCookie("AUTHENUSER",username+"."+response, 1);
                                },
                        ...
                        $('#node-input-topic').trigger('change');
                    }
                },
                error: function(xhr, status, err) {
                    console.log(xhr.responseText);
            ...
            // $('#node-input-name').val('MobifoneITC');
            // var authen = 'Bearer eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJ0dGNudHRAbW9iaWZvbmUudm4iLCJpYXQiOjE2MjYxNDY1NTAsImV4cCI6MTYyNjIzMjk1MH0.0s7XwYLg7mMGQfx5f0vvYWcNgsh3nXhXMoKJlqgj3uSwEHo-J_U1IGD8-sNoIaJdwHCpie7foYqB2hDlL2BVMg'
            // fetch(apiGetAll, {
            //     method: "GET",
            //     //   mode:"no-cors",
            //     //   credentials: 'include',
...
<script type="text/html" data-template-name="mqtt out">
    ...
    <div class="form-row mqtt-form-row-cols2">
        <label for="node-input-qos" class="mqtt-form-row-col1"><i class="fa fa-empire"></i> <span
                data-i18n="mqtt.label.qos"></span></label>
        ...
        <label for="node-input-retain" class="mqtt-form-row-col2"><i class="fa fa-history"></i> <span
                data-i18n="mqtt.retain"></span></label>
    ...
    <script type="text/javascript">
        ...
        var loadData = function () {
            $.ajax({
                type: 'GET',
                url: 'http://10.16.150.139:8000/getChannelsByUser',
                success: function(response) {
                    // console.log("QUANGLISTCHANNELS:"+response);
                    var listChannel = response.data
                    for (var i = 0; i < listChannel.length; i++) {
                        ...
                        document.getElementById("node-input-name").value = "Mobifone Channel Out";
                        var a = document.getElementById('node-input-topic');
                        $('#node-input-topic').change(function () {
                            // alert(this.value);
                            // console.log(a.selectedIndex);
                            var username = listChannel[a.selectedIndex].thing_id;
                            var password = listChannel[a.selectedIndex].thing_key;
                            console.log('http://10.16.150.139:8000/encrypt/'+password)
                            $.ajax({
                                type: 'GET',
                                url: 'http://10.16.150.139:8000/encrypt/'+password,
                                success: function(response) {
...
<script type="text/html" data-template-name="mqtt-broker">
    ...
    <div id="node-config-mqtt-broker-tabs-content" style="min-height:150px;">
        <div id="mqtt-broker-tab-connection" style="display:none">
            <div class="form-row node-input-broker">
                ...
                <input type="text" id="node-config-input-broker" style="width: calc(100% - 300px);"
            ...
            <div class="form-row" style="height: 34px;">
                ...
                <label for="node-config-input-usetls" style="width: 100px; line-height: 34px;"><span
                        data-i18n="mqtt.label.use-tls"></span></label>
                <span id="node-config-row-tls" class="hide"><input style="width: 320px;" type="text"
                                                                   id="node-config-input-tls"></span>
            </div>
            <div class="form-row">
                <label for="node-config-input-protocolVersion"><i class="fa fa-cog"></i> <span
                        data-i18n="mqtt.label.protocolVersion"></span></label>
            ...
            <div class="form-row">
                <label for="node-config-input-keepalive"><i class="fa fa-heartbeat"></i> <span
            ...
            <div class="form-row" style="margin-bottom:0">
                <label style="vertical-align:top;"><i class="fa fa-info"></i> <span
                        data-i18n="mqtt.label.session"></span></label>
                <div style="display: inline-block; width:calc(100% - 110px)">
                    ...
                    <div class="form-row mqtt5">
                        <label style="width:auto" for="node-config-input-sessionExpiry"><span
                                data-i18n="mqtt.label.sessionExpiry"></span></label>
                        <input type="number" min="0" id="node-config-input-sessionExpiry" style="width: 100px">
                    </div>
                </div>
            </div>
            <div class="form-row mqtt5">
                <label style="width: 125px;" for="node-config-input-userProps"><span
                        data-i18n="mqtt.label.userProperties"></span></label>
                <input type="text" id="node-config-input-userProps" style="width: calc(100% - 166px);">
            </div>
            <br>
        </div>
        <!--        //SECURITY-->
        <div id="mqtt-broker-tab-security">
            <div class="form-row">
            ...
            <div id="mqtt-broker-section-birth">
                ...
                <div class="section-content" style="padding:10px 0 0 10px">
                    ...
                    <div class="form-row">
                        ...
                        <input style="width: calc(100% - 300px) !important" type="text"
                               id="node-config-input-birthPayload" style="width:300px"
                               data-i18n="[placeholder]common.label.payload">
                    ...
                    <div class="form-row mqtt5 mqtt5-out">
                        <label for="node-config-input-birth-respTopic"><span
                                data-i18n="mqtt.label.responseTopic"></span></label>
                ...
                <div class="section-content" style="padding:10px 0 0 10px">
                    <div class="form-row">
                        <label style="width: 100px !important;" for="node-config-input-closeTopic"><i
                                class="fa fa-tasks"></i> <span data-i18n="common.label.topic"></span></label>
                        <input style="width: calc(100% - 300px) !important" type="text"
                               id="node-config-input-closeTopic" style="width:300px"
                    ...
                    <div class="form-row">
                        <label style="width: 100px !important;" for="node-config-input-closePayload"><i
                                class="fa fa-envelope"></i> <span data-i18n="common.label.payload"></span></label>
                        <input style="width: calc(100% - 300px) !important" type="text"
                               id="node-config-input-closePayload" style="width:300px"
                               data-i18n="[placeholder]common.label.payload">
                    ...
                    <div class="form-row mqtt5 mqtt5-out">
                        <label for="node-config-input-close-respTopic"><span
                                data-i18n="mqtt.label.responseTopic"></span></label>
                ...
                <div class="section-content" style="padding:10px 0 0 10px">
                    <div class="form-row">
                        ...
                        <input style="width: calc(100% - 300px) !important" type="text" id="node-config-input-willTopic"
                               style="width:300px" data-i18n="[placeholder]mqtt.placeholder.will-topic">
                    ...
                    <div class="form-row">
                        <label style="width: 100px !important;" for="node-config-input-willPayload"><i
                                class="fa fa-envelope"></i> <span data-i18n="common.label.payload"></span></label>
                        <input style="width: calc(100% - 300px) !important" type="text"
                               id="node-config-input-willPayload" style="width:300px"
                               data-i18n="[placeholder]common.label.payload">
    ...
    <script type="text/javascript">
        ...
        // console.log("QUANGCOKIIE: "+readCookie("AUTHENUSER"))
        // $(document ).ready(function() {
        //     $("#node-config-input-user").val("QUTESSTTTT")
        // });
        // document.getElementById("node-config-input-user").value = "Johnny Bravo";

        // console.log("QUANG: " + readCookie('AUTHENUSER'))
        // console.log(document.getElementById("node-config-input-user").value)
        // console.log(document.getElementById("node-config-input-user"))
        // // $('#node-config-input-user').value = "Johnny Bravo";
        // $('#node-config-input-user').load(() => {
        //     alert('123');
        //     $('#node-config-input-user').value = "Johnny Bravo";
        // });
        // function test(){
        ...
        //     alert('12ssss3');
        //     document.getElementById("node-config-input-user").value = "value";
        // }
            ...
            oneditprepare: function () {
                var tabs = RED.tabs.create({
                    ...
                    onchange: function(tab) {
                        ...
                        $("#" + tab.id).show();
                        // if(tab.id == 'mqtt-broker-tab-security'){

                        $('#node-config-input-broker').val('10.16.150.132');
                            var res = readCookie('AUTHENUSER');
                            if(!!res){
                                var arrRes = res.split(".")
                                $('#node-config-input-user').val(arrRes[0]);
                                var passEncrypt = arrRes[1];
                                $.ajax({
                                    type: 'GET',
                                    url: 'http://10.16.150.139:8000/decrypt/'+passEncrypt,
                                    success: function(response) {
                                        // console.log("AFTER"+response);
                                        $('#node-config-input-password').val(response);
                                    },
                                    error: function(xhr, status, err) {
                                        console.log(xhr.responseText);
                                    }
