...
var settingconfigNode = require('../../../../../Settings')
// console.log(settingConfig.getUrlSaveFlow())
 ...
 */
function setFlows(_config,_credentials,type,muteLog,forceStart,user,req) {
    ...
    if (type === "load") {
        ...
        // console.log("CHIQUANG: "+req.session.token)
...
// async function getFlowFromDB(token,flowid,activeFlow) {
//     token = `Bearer eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJOb2RlUmVkVXNlckBtb2JpZm9uZS52biIsImlhdCI6MTYyMjEwNzQzNywiZXhwIjoxNjIyMTkzODM3fQ.BEBxT0yAiI62adnoqSdWtCyhGj_VqwZPBdqMs2o4rscPfg0XknNbKc4yvlZ0i6y5wouViTyw6FqwyoRMlKjZAg`
//     // console.log("QUANG: "+settingconfigNode.getUrlSaveFlow())
//     var http = require('http');
//     const options = {
//         hostname: settingconfigNode.getHostNodered(),
//         port: settingconfigNode.getPortNodered(),
//         path: settingconfigNode.getUrlGetFlow()+'?flowId='+`${flowid}`,
//         method: 'GET',
//         headers: {
//             Authorization: `${token}`
//         }
...
//             activeFlow = JSON.stringify(rs);
//             // return rs
//             console.log("QUANGKAKAKAKA: "+activeFlow);
//         });
//     }
...
// }
    ...
    //NODE-RED LOAD
 function getFlows(req,type) {
    if(type==="loadFlows"){
        console.log("LOAD FLOWS")
        var token = req.session.token
        var flowid = req.session.flowid
        console.log('http://'+settingconfigNode.getHostNodered()+':'+settingconfigNode.getPortNodered()+settingconfigNode.getUrlGetFlow()+'/'+`${flowid}`)
        var res = requestSync('GET', 'http://'+settingconfigNode.getHostNodered()+':'+settingconfigNode.getPortNodered()+settingconfigNode.getUrlGetFlow()+'/'+`${flowid}`, {
            headers: {
        ...
        try{
            ...
            if ( res instanceof Error || typeof finalData === "undefined" || finalData === "{}") {
                var initFlow = ' [{"id":"8d053e0c.2f2ef","type":"tab","label":"Flow 1","disabled":false,"info":""}]'
                activeConfig.flows = initFlow
            }
            else{
                activeConfig.flows = JSON.parse(res.getBody('utf8'))
            }
        } catch (e){
            console.log("LOIIIIII JSON")
            var initFlow = ' [{"id":"8d053e0c.2f2ef","type":"tab","label":"Flow 1","disabled":false,"info":""}]'
            activeConfig.flows = initFlow
        }
